...
from cyberaccess_sdk import CyberAccessClient, AsyncCyberAccessClient
from cyberaccess_sdk.fastapi import enforce, async_enforce

BASE = "https://api.example.test"

guard = CyberAccessClient(api_key="sk_test", base_url=BASE)
async_guard = AsyncCyberAccessClient(api_key="sk_test", base_url=BASE)
app = FastAPI()
...
@app.get("/async-records/{record_id}")
async def get_async_record(record_id: str, subject: str = "alice", is_owner: bool = True):
    result = await async_enforce(async_guard, subject=subject, resource_id=record_id, authorized=is_owner)
    return {"record_id": record_id, "risk_score": result.score}
...
@respx.mock
def test_async_route_allows_when_engine_allows():
    respx.post(f"{BASE}/v1/authorize").mock(
        return_value=httpx.Response(200, json={"decision": "allow", "score": 0, "category": "Normal", "signals": []})
    )
    res = client.get("/async-records/1")
    assert res.status_code == 200
    assert res.json()["risk_score"] == 0
