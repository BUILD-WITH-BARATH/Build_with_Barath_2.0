...
* **Identity is unauthenticated.** The `X-Subject` header is a stand-in for a real auth system (session/JWT) and is trivially spoofable. It exists to demonstrate the object-authorization and behavioral-detection logic in isolation, not to be a production identity layer.
...
* **`/reset` and `/simulate/*` are unauthenticated** on the demo backend by design, so the dashboard's Reset and Attack Simulator buttons work without login. This is fine for a local/sandboxed demo; it would need an admin gate before any public deployment.
* **State is in-memory and ephemeral.** The behavioral risk engine's sliding windows live in a single process's memory — a restart clears all risk history, and the design doesn't yet address horizontal scaling (multiple API instances would each track risk independently).
