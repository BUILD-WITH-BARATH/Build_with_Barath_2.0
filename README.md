...
* **Identity is now real, but demo-grade.** `POST /auth/register` + `POST /auth/login` issue signed JWT bearer tokens (HS256, bcrypt-hashed passwords) and every protected endpoint verifies them server-side — the old trivially-spoofable `X-Subject` header is gone. `JWT_SECRET`, `DEMO_PASSWORD`, and `ADMIN_PASSWORD` are dev-only defaults read from environment variables; rotate them before any shared deployment.
* **All traffic is synthetic.** Every number in this README (0% false positives, detection thresholds, etc.) comes from data we generated ourselves via `/simulate/*` and `benchmark.py`. It has not been validated against real-world API traffic. See `bola-benchmark/EXTERNAL_VALIDATION.md` for the protocol we use to get an independent, non-self-graded rerun.
* **`/reset` and `/simulate/*` stay unauthenticated** on the demo backend by design (rate-limited, not auth-gated), so the dashboard's Reset and Attack Simulator buttons work without login. This is fine for a local/sandboxed demo; it would need an admin gate before any public deployment. Every genuinely sensitive endpoint (`/admin/*`, `/soc/*`, `/audit-events`) now requires a `security_admin`-role JWT.
...
* **A basic transport-layer rate limiter is in place** (`slowapi`, keyed by authenticated subject where available, IP otherwise) as a first line of defense independent of the behavioral heuristics. Limits are set generously for demo/test traffic, not tuned for production load.
* **The audit log is now append-only and separate from the demo dataset** (`audit.db`, distinct file from `demo.db`) so `/reset` can no longer wipe incident history along with the seed data.
* **No live/hosted demo yet.** Both services currently need to be run locally (`uvicorn` + `npm run dev`); there is no deployed URL.
* **SQLite is a demo choice, not a scale choice.** The dataset (100 records, ~70 users) is small and fits in a single file; the authorization queries would need to be re-validated against a production-scale schema and index plan.
